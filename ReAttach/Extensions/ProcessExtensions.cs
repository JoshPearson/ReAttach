...
namespace ReAttach.Extensions
{
   public static class ProcessExtensions
   {
      ...
      {
         //Does not take the process username into consideration :
         //return string.Compare(process.Name, target.ProcessPath, StringComparison.OrdinalIgnoreCase) == 0;
         return string.Equals(process.Name, target.ProcessPath, StringComparison.OrdinalIgnoreCase) 
            && string.Equals(process.UserName, target.ProcessUser, StringComparison.OrdinalIgnoreCase);
